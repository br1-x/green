...
'''
        ...
        markup = InlineKeyboardMarkup().row(InlineKeyboardButton(text='🗝 Открыть товар', url=f'https://t.me/GrenShopBot?start={product_id}'))
        item = InlineQueryResultArticle(id=result_id, title=f"Отправить товар", thumb_url='https://imgur.com/a/lyNqELJ', input_message_content=input_content, reply_markup=markup)
...
async def inline_ad_handler(q: InlineQuery):
    text = f'''<b>🛍 GREEN shop является магазином электронных товаров 
...
🤖 GREEN SHOP: @GrenShopBot</b>
'''
    result_id: str = md5(q.query.encode()).hexdigest()
    markup = InlineKeyboardMarkup().row(InlineKeyboardButton(text='📌 Вступить в бота', url=f'https://t.me/GrenShopBot?start={q.query.split(":")[1]}'))
    item = InlineQueryResultPhoto(id=result_id, title=f"Отправить пост", photo_url='https://imgur.com/upload', thumb_url='https://imgur.com/a/lyNqELJ', caption=text, reply_markup=markup)
    await bot.answer_inline_query(q.id, [item], is_personal=True, cache_time=5)
...
На какую сумму вы хотите пополнить баланс? (Минимум 100 RUB)

Ваш текущий баланс: {user.balance} RUB"""
    
    await call.message.edit_caption(caption=text)
    await call.message.answer("💳 Введите сумму для пополнения (в RUB):")
...
async def process_stars_amount(message: types.Message, state: FSMContext):
    try:
        ...
        prices = [
            types.LabeledPrice("Пополнение баланса", amount * 100)  # amount in kopeks
        ]
        
        await message.bot.send_invoice(
            chat_id=message.chat.id,
            title=f"Пополнение баланса на {amount} ⭐",
            description=f"Пополнение баланса в боте на {amount} ⭐",
        ...
        await message.answer("❌ Пожалуйста, введите корректную сумму в рублях (только цифры)")
...
Выберите пакет звезд для пополнения баланса:

• 1000 ⭐ - 1000 RUB
• 5000 ⭐ - 4500 RUB (10% скидка)
• 10000 ⭐ - 8000 RUB (20% скидка)

После оплаты звёзды будут автоматически зачислены на ваш баланс."""
    
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("10 ⭐ - 1000 RUB", callback_data="stars_package:10:1000"))
    markup.add(InlineKeyboardButton("50 ⭐ - 4500 RUB (-10%)", callback_data="stars_package:50:4500"))
    markup.add(InlineKeyboardButton("100 ⭐ - 8000 RUB (-20%)", callback_data="stars_package:100:8000"))
    markup.add(InlineKeyboardButton("👨‍💼 Связаться с администратором", url="https://t.me/admin_username"))
    markup.add(InlineKeyboardButton("⬅️ Назад", callback_data="stars_pop"))
...
async def topup_profile_handler(call: types.CallbackQuery, state: FSMContext):
    user = User(call.from_user.id)
    if user.balance < 1:
        await call.answer("❌ У вас недостаточно звезд для пополнения профиля", show_alert=True)
        return
...
async def process_stars_to_balance(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if stars > user.balance:
            await message.answer(f"❌ У вас недостаточно звезд. Доступно: {user.balance} ⭐")
            return
        ...
        await message.answer(
            f"✅ Успешно переведено {stars} ⭐ в баланс профиля\n"
            f"Новый баланс: {user.balance} ⭐"
        )
...
async def stars_help_handler(call: types.CallbackQuery, state: FSMContext):
    text = """<b>❓ Как работают звезды?</b>

1. <b>Звезды</b> - это внутренняя валюта бота
2. Вы можете купить звезды за реальные деньги
3. 1 ⭐ = 1 RUB при пополнении профиля
4. Звездами можно пополнять баланс профиля в любое время
...
• Скидки при покупке больших пакетов"""
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🛒 Купить звезды", callback_data="buy_stars"))
    markup.add(InlineKeyboardButton("⬅️ Назад", callback_data="stars_pop"))
...
◽️ID: {c.from_user.id}
◽️Баланс пользователя: {User(c.from_user.id).balance} ⭐
</b>
"""
...
<b>Если у вас есть все же сомнения над нашим ботом приглашаем вас в канал отзывов </b><b>(<a href='https://t.me/end_soft'>тыкай</a>)</b>"""
            ...
            if call.data == 'crypto_curs':
                data = get("https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,ETH,LTC,USDT,DOGE,DOT&tsyms=USD,⭐").json()
                txxt = f'''<b>📈 Валютные котировки:</b>

<b>▪️BTC</b> - {data["BTC"]["USD"]} $ | {data["BTC"]["⭐"]} ₽
<b>▪️ETH</b> - {data["ETH"]["USD"]} $ | {data["ETH"]["⭐"]} ₽
<b>▪️LTC</b> - {data["LTC"]["USD"]} $ | {data["LTC"]["⭐"]} ₽
<b>▪️USDT</b> - {data["USDT"]["USD"]} $ | {data["USDT"]["⭐"]} ₽
<b>▪️DOGE</b> - {data["DOGE"]["USD"]} $ | {data["DOGE"]["⭐"]} ₽
<b>▪️DOT</b> - {data["DOT"]["USD"]} $ | {data["DOT"]["⭐"]} ₽
'''
...
🔗 Ссылка для входа: https://t.me/+YpqU_GLITatkMmFi""", reply_markup=menu.back_markup())
                else:
